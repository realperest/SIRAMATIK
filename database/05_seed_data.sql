...
INSERT INTO siramatik.firmalar (id, ad, logo_url, ayarlar) VALUES
(
...
-- 2. Servisler Ekle
INSERT INTO siramatik.servisler (firma_id, ad, kod, aciklama) VALUES
...
DECLARE
    ...
    phr_id UUID;
BEGIN
    SELECT id INTO lab_id FROM siramatik.servisler WHERE kod = 'LAB';
    SELECT id INTO reg_id FROM siramatik.servisler WHERE kod = 'REG';
    SELECT id INTO phr_id FROM siramatik.servisler WHERE kod = 'PHR';
    
    -- Laboratuvar servisi altında 3 kuyruk
    INSERT INTO siramatik.kuyruklar (servis_id, ad, kod, aciklama, oncelik) VALUES
...
INSERT INTO siramatik.kullanicilar (firma_id, email, ad_soyad, rol, sifre_hash) VALUES
(
    '11111111-1111-1111-1111-111111111111',
    'admin@demo.com',
    'Admin User',
...
BEGIN
    SELECT id INTO lab_id FROM siramatik.servisler WHERE kod = 'LAB';
    SELECT id INTO lab_blood_queue FROM siramatik.kuyruklar WHERE kod = 'A' AND servis_id = lab_id;
    SELECT id INTO lab_urine_queue FROM siramatik.kuyruklar WHERE kod = 'B' AND servis_id = lab_id;
    
    INSERT INTO siramatik.cihazlar (firma_id, servis_id, kuyruk_id, ad, tip, konum) VALUES
...
BEGIN
    ...
    JOIN siramatik.servisler s ON k.servis_id = s.id 
    ...
    FOR i IN 1..5 LOOP
        INSERT INTO siramatik.siralar (
            ...
            lab_blood_queue,
            (SELECT servis_id FROM siramatik.kuyruklar WHERE id = lab_blood_queue),
            '11111111-1111-1111-1111-111111111111',
    ...
    FOR i IN 1..3 LOOP
        INSERT INTO siramatik.siralar (
            ...
            lab_urine_queue,
            (SELECT servis_id FROM siramatik.kuyruklar WHERE id = lab_urine_queue),
            '11111111-1111-1111-1111-111111111111',
            'B' || LPAD(i::TEXT, 3, '0'),
            CASE 
    ...
    INSERT INTO siramatik.siralar (kuyruk_id, servis_id, firma_id, numara, durum, oncelik) VALUES
    (phr_vip_queue, (SELECT servis_id FROM siramatik.kuyruklar WHERE id = phr_vip_queue), '11111111-1111-1111-1111-111111111111', 'VIP001', 'waiting', 9),
    (phr_vip_queue, (SELECT servis_id FROM siramatik.kuyruklar WHERE id = phr_vip_queue), '11111111-1111-1111-1111-111111111111', 'VIP002', 'waiting', 9);
    
END $$;

-- 7. Sistem Ayarları
INSERT INTO siramatik.sistem_ayarlari (anahtar, deger, aciklama) VALUES
