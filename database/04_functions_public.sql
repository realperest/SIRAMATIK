...
CREATE OR REPLACE FUNCTION yeni_sira_numarasi(p_kuyruk_id UUID, p_oncelik INT DEFAULT 0)
RETURNS VARCHAR AS $$
DECLARE
...
COMMENT ON FUNCTION yeni_sira_numarasi IS 'Kuyruk için otomatik sıra numarası üretir. Öncelikli sıralar VIP öneki alır.';

-- 2. Bekleyen sıra sayısını getir (önceliğe göre)
CREATE OR REPLACE FUNCTION bekleyen_sira_sayisi(p_kuyruk_id UUID, p_oncelik INT DEFAULT NULL)
RETURNS INT AS $$
BEGIN
...
COMMENT ON FUNCTION bekleyen_sira_sayisi IS 'Kuyruktaki bekleyen sayısını döndürür. Öncelik belirtilirse sadece o önceliği sayar.';

-- 3. Sıradaki kişiyi getir (öncelik sırasına göre)
CREATE OR REPLACE FUNCTION siradaki_kisi(p_kuyruk_id UUID)
RETURNS TABLE (
    sira_id UUID,
    numara VARCHAR,
    oncelik INT,
...
COMMENT ON FUNCTION siradaki_kisi IS 'Kuyruktaki sıradaki kişiyi getirir (öncelik > zaman sırası)';

-- 4. Ortalama bekleme süresi (dakika)
CREATE OR REPLACE FUNCTION ortalama_bekleme_suresi(p_kuyruk_id UUID, p_gun_sayisi INT DEFAULT 7)
RETURNS INT AS $$
DECLARE
    ort_saniye NUMERIC;
BEGIN
...
COMMENT ON FUNCTION ortalama_bekleme_suresi IS 'Son N gündeki ortalama bekleme süresini dakika olarak döndürür';

-- 5. Günlük kuyruk istatistikleri
CREATE OR REPLACE FUNCTION gunluk_istatistikler(p_firma_id UUID, p_tarih DATE DEFAULT CURRENT_DATE)
RETURNS TABLE (
    servis_ad VARCHAR,
    kuyruk_ad VARCHAR,
    toplam_sira BIGINT,
...
BEGIN
    ...
    FROM servisler s
    LEFT JOIN kuyruklar k ON s.id = k.servis_id
    LEFT JOIN siralar q ON k.id = q.kuyruk_id 
...
COMMENT ON FUNCTION gunluk_istatistikler IS 'Belirli bir gün için kuyruk bazlı istatistikler (VIP sayısı dahil)';

-- 6. Eski sıraları temizle (GDPR/KVKK uyumu)
CREATE OR REPLACE FUNCTION eski_siralari_temizle(p_gun_sayisi INT DEFAULT 180)
RETURNS INT AS $$
DECLARE
    silinen_sayisi INT;
BEGIN
...
COMMENT ON FUNCTION eski_siralari_temizle IS 'Belirtilen günden eski tamamlanmış sıraları siler (varsayılan: 180 gün)';
